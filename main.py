...
def trackBox(frame, bbox):
    ...
    while True:
        ...
        if cur_x > x+(screenWidth/2 - 80) and cur_x < x+(screenWidth/2 - 80)+w and cur_y > y+(screenHieght/2 - 80) and cur_y < y+(screenHieght/2 - 80)+h:
            if shootLockedTarget == True:
                pyautogui.click()
            break
        else:
            drawBox([bbox])
